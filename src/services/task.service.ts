...
import type { TaskStatus, TaskPriority, ActivityType } from '@prisma/client';
...
export class TaskService {
  ...
  async update(taskId: string, userId: string, data: UpdateTaskData) {
    ...
    if (changes.length > 0) {
      await this.createActivity(
        userId,
        taskId,
        data.status && data.status !== task.status ? 'STATUS_CHANGED' : 'UPDATED',
  ...
  async moveTask(
    ...
    await this.createActivity(
      ...
      taskId,
      'MOVED',
      `moved task from ${task.column?.name ?? 'Unknown'} to ${targetColumn.name}`
    );
  ...
  async unassignUser(taskId: string, userId: string, assigneeId: string) {
    ...
    await this.createActivity(
      ...
      taskId,
      'UNASSIGNED',
      `unassigned ${assignee?.firstName} ${assignee?.lastName}`
    );
