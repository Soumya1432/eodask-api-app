...
export class OrganizationService {
  ...
  async create(userId: string, data: CreateOrganizationData) {
    ...
    if (ownedCount >= MAX_OWNED_ORGANIZATIONS) {
      throw ApiError.badRequest(
        `You can only create up to ${MAX_OWNED_ORGANIZATIONS} organizations`
      );
    }
  ...
  async update(orgId: string, userId: string, data: UpdateOrganizationData) {
  ...
  async updateSlug(orgId: string, userId: string, newSlug: string) {
    const org = await this.findById(orgId, userId);
  ...
  async delete(orgId: string, userId: string) {
    ...
    if (org._count.projects > 0) {
      throw ApiError.badRequest(
        'Cannot delete organization with projects. Delete all projects first.'
      );
    }
  ...
  async updateSettings(orgId: string, userId: string, data: UpdateSettingsData) {
    const org = await this.findById(orgId, userId);
  ...
  async uploadLogo(orgId: string, userId: string, logoUrl: string) {
    const org = await this.findById(orgId, userId);
  ...
  async addMember(orgId: string, userId: string, memberEmail: string, role: OrganizationRole) {
    const org = await this.findById(orgId, userId);

    // Check if user can add members (admin+ or member if allowed)
    const canAdd =
      this.hasMinRole(org.userRole, 'ADMIN') ||
  ...
  async acceptInvitation(token: string, userId: string) {
    ...
    if (user?.email !== invitation.email) {
      throw ApiError.forbidden(
        `This invitation was sent to ${invitation.email}. Please log in with that email address to accept the invitation.`
      );
    }
  ...
  // Dashboard stats
  async getDashboardStats(orgId: string, userId: string) {
    await this.findById(orgId, userId);
  ...
  async getProjects(orgId: string, userId: string, page = 1, limit = 10) {
    await this.findById(orgId, userId);
