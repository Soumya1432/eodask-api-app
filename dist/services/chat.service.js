...
const formatParticipants = (participants) => participants.map(p => ({
    id: p.id,
    userId: p.userId,
    roomId: p.chatRoomId,
...
export class ChatService {
    async createRoom(name, projectId, participantIds, isGroup) {
    ...
    async addParticipant(roomId, userId, newParticipantId) {
    ...
    async removeParticipant(roomId, userId, participantId) {
        const room = await this.getRoomById(roomId, userId);
        if (!room.isGroup) {
    ...
    async getUnreadCount(userId) {
        ...
        for (const room of rooms) {
            ...
            if (participant) {
                const unreadCount = room.messages.filter((m) => m.createdAt > participant.lastReadAt && m.senderId !== userId).length;
