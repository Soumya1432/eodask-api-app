...
class AnalyticsService {
    async getOrganizationAnalytics(organizationId, userId) {
        ...
        const tasksLastWeek = allTasks.filter(t => t.createdAt >= startOfLastWeek && t.createdAt < startOfThisWeek).length;
        ...
        for (let i = 7; i >= 0; i--) {
            ...
            const created = allTasks.filter(t => t.createdAt >= weekStart && t.createdAt < weekEnd).length;
            // Use updatedAt for DONE tasks as completion date
            const completed = allTasks.filter(t => t.status === 'DONE' && t.updatedAt >= weekStart && t.updatedAt < weekEnd).length;
            weeklyTrend.push({
