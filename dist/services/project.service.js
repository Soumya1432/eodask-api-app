...
export class ProjectService {
    ...
    async addMember(projectId, userId, memberEmail, role) {
        const project = await prisma.project.findUnique({
            where: { id: projectId },
        ...
        if (!user) {
            ...
            try {
                await emailService.sendInvitation(memberEmail, project.name, senderName, invitation.token);
            }
            catch (error) {
        ...
        try {
            await emailService.sendTaskAssignmentNotification(user.email, `${user.firstName} ${user.lastName}`, `Added to project: ${project.name}`, project.name, `${config.clientUrl}/projects/${projectId}`);
        }
        catch (error) {
    ...
    async updateMemberRole(projectId, userId, memberId, role) {
        const project = await prisma.project.findUnique({
            where: { id: projectId },
            include: { members: true },
        });
    ...
    async getColumns(projectId, userId) {
        // Check if user has access to project
        await this.findById(projectId, userId);
    ...
    async updateColumn(projectId, userId, columnId, data) {
        // Check if user has access to project
        await this.findById(projectId, userId);
    ...
    async reorderColumns(projectId, userId, columnOrders) {
        // Check if user has access to project
        await this.findById(projectId, userId);
        const updates = columnOrders.map((col) => prisma.boardColumn.update({
            where: { id: col.id },
